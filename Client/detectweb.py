...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.Ready_on = QTimer(self, interval=1000)
        self.Ready_on.timeout.connect(self.Client_lamp)
    ...
    def Client_lamp(self):
        if self.Status_client:
            self.serverofflabel.setStyleSheet("background-color: rgb(255, 195, 0); border-radius:30px")
        else:
            self.serverofflabel.setStyleSheet("background-color: rgb(242, 214, 117); border-radius:30px")
        self.Status_client = not self.Status_client
    ...
    def alarm_stop(self):
        ...
        message_json = json.dumps(message)
        self.client_socket.sendall(message_json.encode('utf-8'))
        self.datasendlabel.setStyleSheet("background-color: rgb(85, 255, 0); border-radius:30px")
    ...
    def get_drowsiness(self, counter):
        ...
        if counter > 3 and not self.sent_signal:
            ...
            self.datasendlabel.setStyleSheet("background-color: rgb(184, 230, 191); border-radius:30px")
